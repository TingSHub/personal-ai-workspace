...
| 2026-08-16 | 边界收紧 v0.1.1：①不输出具体目标价（机构或自研）与买卖评级，盈利一致预期（净利/EPS）保留为市场预期；②HTML 不得展示内部置信度/证据编号/证据台账；③避免「唯一/无懈可击/绝对」等绝对化措辞；④「企稳确认/压力信号」确认为允许内容（基本面验证性质，不写成买卖指令） | 用户边界修正（2026-08-16 复评） |
| 2026-08-16 | v0.1.5：经验沉淀回写——notes-spoken-polish（口语化陷阱+零漂移脚本）、narration-content-lock（notes 唯一 SoT）、presenter-notes-verification（S 键回归验证）；P3 已知问题补充口语化陷阱 | 经验: `notes-spoken-polish`、`narration-content-lock`、`presenter-notes-verification` |
| 2026-08-17 | v0.1.6：Phase 3 增加 Core Message / Visual Evidence / Narration Expansion 对齐门禁；要求重要口播有视觉锚点，但不要求逐字上屏 | 用户方法论补充（认知一致而非文字一致） |
| 2026-08-17 | v0.1.7：Phase 3 增加 HTML Deck 分阶段呈现约束；每页定义核心结论、主视觉、阶段步骤、元素变化和 narration-to-stage 映射，动效实现方式保持自由 | 用户方法论补充（旁白驱动的阶段化呈现） |
| 2026-08-18 | v0.1.8：Phase 2 改由 financial-editor-agent 主责；从单纯冲突清单升级为 Editorial Pitch + Editorial Master + 冲突/不确定性记录，Phase 3 改为优先消费 Editorial Master | 用户指令（优化 Phase 2 并交给 Financial Editor Agent） |
| 2026-08-18 | v0.1.9：Phase 3 改由 finance-resentation-agent 主责；html-ppt-skill 降为实现能力，新增 Scene/Core Message、Visual Evidence、Notes/Reveal、执行留痕与选择门禁 | 用户指令（优化 Phase 3 并交给 Finance Presentation Agent） |
| 2026-08-18 | v0.2.0：Phase 3 增加 Presentation Completeness Principle；关键认知步骤优先于 Scene 数量，Reveal 只展开同一认知模型；移除 dashi-ppt 可选资源 | 用户指令（完整性优先与 Reveal 边界） |
| 2026-08-19 | v0.2.1：Phase 3/4 增加可选视觉设计资源注记——taste-skill（视觉方向参考，editorial 反模板纪律）与 impeccable（audit/polish 质量层，确定性检测无 LLM，纯命令调用无 hook）；两者只做质量增强层，不驱动内容与设计决策（Read 模式 + 内容边界压制激进表达） | 用户指令（注册并集成两个外部设计 skill：pbakaus/impeccable、Leonxlnx/taste-skill） |
| 2026-08-19 | v0.3.0：Phase 3 确立三资源职责分工模型（不新增 Phase，均在 Phase 3 内）——内容导演层 finance-resentation-agent（Scene/Core Message/认知顺序/Visual Evidence/是否拆页/Reveal Semantics/Notes）→ 视觉语言层 impeccable（视觉语言/版式/字体层级/图表美化/信息层级/设计系统/细节质感，由可选升级为必选）→ 工程实现层 html-ppt-skill（HTML Deck/Presenter/Notes/Stage/runtime）；impeccable 明确「只做视觉不驱动内容决策」；Known Issues 沉淀同名类污染/stage 动画位移/矮视口溢出/counter 类名/detect 字号阈值等实测踩坑 | 用户指令（三层分工模型；impeccable 实测显著拉高设计质量，升级为必选视觉层）+ 2026-08-19 茅台翻页 deck 实测 |
| 2026-08-19 | v0.4.0：Finance Presentation Agent 升级为 HyperFrames-native Presentation Director；增加 Composition 运行时层、共享 Scene DOM/SVG、seek-safe 状态、Reveal Cue、Composition QA，并将 Oral Adaptation 与 Notes 唯一 SoT 锁定在 Presentation 阶段 | 用户指令（整合 Finance Presentation Agent 职责与 HyperFrames 视频生产契约） |
