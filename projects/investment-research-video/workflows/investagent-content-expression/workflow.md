...
把已验收的事实研究包和已 closed 的内容协作稿变成有明确判断、自然双人播客表达和增长承诺的锁定内容包。先在同一份协作稿中生成或修订母稿，再进入导演锁定和对白生成。按 `content_depth` 组织短机制课或深度解释型节目：深度模式保留一条核心机制，但用历史、新闻和市场预期关系把它讲透。只做编辑判断与表达编排，不补研究、不执行音频或视觉制作。

## Input

- `topic-research` 已验收的 `research-brief.md`、`source-ledger.md`、evidence map 和 `research-quality-gate.md`；
- `topic-research` 已 closed 的 `content-collaboration.md`；
- 用户批准的 topic card，且必须包含 `content_line`、`expression_agent` 和 `expression_mode`；
- `account-profile`、内容政策、对白政策、目标平台和目标时长。
- topic card 或研究包中的 `content_depth`；旧输入缺失时兼容为 `standard`，用户明确要求历史/新闻/股价关系时不得继续按标准模式压缩。
...
- `content-collaboration.md` 是本期唯一的人工协作稿；母稿和口播审核在该文件中原位更新，不为每轮讨论创建新文件。
- `deep_explainer` 必须消费研究包中的 `Knowledge Map`、`Historical And Market Relation` 和事实锚点；表达层可以重排、翻译和调侃，不能自行补充历史或股价事实。
...
- “一条核心机制”不等于“一节五分钟短课”：深度模式可以有概念、历史、当前新闻、市场关系和验证五个教学模块，但它们必须共同服务同一个核心判断。

## Stage Boundary

```text
research-brief + evidence IDs + content-collaboration.md
        ↓
...
从冻结研究包和已 closed 的协作稿中整理母稿，再选择唯一核心主张、叙事模式、开场、知识路径、模块顺序、增长承诺和结尾条件；深度模式要把历史与市场关系安排成证明链，而不是资料附录。
...
批准 topic card、已验收 research brief/source ledger/evidence map、已 closed 的 content-collaboration.md、account profile、目标平台和时长。

### Output

更新后的 `content-collaboration.md`（含母稿和母稿审核状态）、导演方案、topic order、opening selection、scene intent、episode input、director execution 和 `editorial-lock` 回执。

### Quality Criteria

- `editorial_thesis` 明确回答批准问题，包含机制、时间范围、受影响环节、最强反证、推翻条件和 evidence IDs；
- `content-collaboration.md` 中的母稿必须先完成审核：`manual` 模式下用户与 Agent 均确认后才能进入 `editorial-lock`；
- `check_content_collaboration.py --phase mother` 通过后才能把母稿交给对白导演；
- 只有一个核心判断；每个模块有不同问题、证据推进和 audience payoff；
- `deep_explainer` 必须明确“人话直觉 → 概念 → 历史验证 → 当前新闻/事实 → 市场预期或估值含义 → 验证条件”的学习路径，并说明哪些内容被舍弃；
- 标题或开场若使用“猪股上涨/板块走强”，正文必须兑现一段有时间窗口和来源的猪价—利润—市场预期关系，不能只讲猪价；
...
- `humor-map.md` 每个主要笑点必须记录事实锚点、观众误解或情绪、口语化反差、回到判断的承接句和边界；没有事实锚点的纯段子不进入锁定稿。
...
把已锁定的主张、知识路径和模块编排成真实 `host_analyst` 双人播客逐句稿，并完成统一财经表达工程。深度模式重点检查“事实—反应—幽默—回证据”的听觉节奏。
...
通过 Phase 1 验收的 episode input、director treatment、topic order、研究包、已更新的 content-collaboration.md 和 account profile。

### Output

更新 `content-collaboration.md` 的口播草案与审核意见，并在审核通过后锁定 `episode.json`、dialogue map、spoken style map、spoken polish diff 和 dialogue director execution。

### Quality Criteria

- 每个 turn 明确 speaker、reply_to_turn_id、interaction_type、emotion、delivery、evidence_ids 和 text；
- `manual` 模式下，口播先进入协作稿供用户审核；用户和 Agent 均确认后，才能把 `episode.json` 作为执行层输入；
...
- 口播默认直接陈述事实，不在事实句点名来源主体（来源由 evidence IDs、画面脚注和发布来源清单承担）；只有预测、观点、估计、争议口径或来源身份影响结论时才点名，且必须在执行记录中说明理由；
- 含数字的 turn 必须同时提供 `text`（读音安全）与 `display_text`（阿拉伯数字/百分号/单位展示）且数值等价；
- `deep_explainer` 至少要有一段历史关系、一段当前新闻事实和一段猪价/利润/猪股预期关系；每段都必须有自然转场，不能把它们变成资料播报；
- 幽默节拍遵循“事实锚点 → 主理人反应或调侃 → 分析师解释反差 → 回到机制/证据”，笑点不单独生成第二条投资判断；
- 概念命名只有在能降低记忆负担时才使用，可以输出“不命名”；
- `host_analyst` 不被误写成双方等量辩论；
- `check_podcast_dialogue.py` 通过后，文本、speaker、数字和 evidence IDs 冻结。
- `check_content_collaboration.py --phase spoken` 通过后，才允许把 `episode.json` 交给执行层。
...
只有 `editorial-lock`、母稿审核和 `dialogue-lock` 都通过，才移交 `investagent-video-execution`。执行层不得消费 draft，只消费锁定的 `episode.json` 和交接回执。
