#!/usr/bin/env python3
"""Build a podcast episode from a subject manifest only.

Usage:
  python3 scripts/build_podcast_episode.py --input editorial/episode-input.json --run-root <subject-run>

The builder deliberately has no subject/topic/outro fallback. Every spoken line,
...
def main() -> int:
    ...
    growth_contract = manifest.get("growth_contract") or {}
    thesis_contract = manifest.get("thesis_contract") or {}
    required_meta = {"episode_id", "research_date", "show_name", "role_map", "speakers"}
    missing = required_meta - set(meta)
    subject_name = meta.get("subject_name") or meta.get("company_name")
    subject_id = meta.get("subject_id") or meta.get("company_id")
    subject_type = meta.get("subject_type") or ("company" if meta.get("company_name") else None)
    if missing or not subject_name or not subject_type or not opening.get("cold_open") or not topics or not outro:
        raise SystemExit(f"manifest incomplete: missing_meta={sorted(missing)} subject/cold_open/topics/outro required")
    if not manifest.get("editorial_thesis"):
        raise SystemExit("manifest incomplete: editorial_thesis required")
    missing_growth = sorted(field for field in GROWTH_FIELDS if not growth_contract.get(field))
    if missing_growth:
        raise SystemExit(f"growth_contract missing {','.join(missing_growth)}")
    missing_thesis = sorted(field for field in THESIS_FIELDS if not thesis_contract.get(field))
    if missing_thesis:
        raise SystemExit(f"thesis_contract missing {','.join(missing_thesis)}")
    ...
    previous = None
    for i, item in enumerate(opening.get("intro", []), 1):
    ...
    episode = {
        **meta,
        ...
        "opening_cards": opening.get("cards", []),
        ...
        "outro_summary": manifest.get("outro_summary", []),
        "turns": turns,
    }
    ...
    (visual_dir / "chart-spec.json").write_text(json.dumps({"version": "v1", "subject_id": subject_id, "subject_name": subject_name, "charts": [{**t["chart"], "topic_id": t["topic_id"]} for t in topics]}, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    with (script_dir / "debate-script.md").open("w", encoding="utf-8") as handle:
        handle.write(f"# {subject_name}｜账本两面主理人播客稿\n\n")
        for turn in turns:
            handle.write(f"[{turn['speaker']}] {turn['text']}\n\n")
    (script_dir / "dialogue-director-execution.md").write_text("# Dialogue Director Execution\n\n- source: episode-input.json\n- topics: manifest-driven\n- outro: manifest-driven\n- subject-specific fallback: none\n", encoding="utf-8")
    print(f"PASS subject={subject_name} topics={len(topics)} turns={len(turns)}")
    return 0
