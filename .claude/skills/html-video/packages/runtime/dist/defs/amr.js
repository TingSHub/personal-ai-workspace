...
async function resolveBundledVela() {
    const envOverride = process.env[VELA_CLI_BIN_ENV]?.trim();
    if (envOverride)
        return envOverride;
    try {
        const mod = (await import('@powerformer/vela-cli'));
        if (typeof mod.resolveVelaCliBin !== 'function')
            return null;
...
export function readVelaProfile() {
    try {
...
const AMR_MODEL_RANK = new Map(['deepseek-v4-flash', 'deepseek-v4-pro', 'claude-opus-4.8', 'claude-sonnet-4.6', 'gpt-5.5', 'gemini-3.1-pro-preview']
    .map((id, i) => [id, i]));
/**
 ...
 */
export async function listAmrModels(resolvedBin) {
