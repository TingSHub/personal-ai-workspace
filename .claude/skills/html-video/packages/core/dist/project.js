...
import { validate as validateGraph, topoSort, DEFAULT_FRAME_DURATION_SEC, } from '@html-video/content-graph';
...
export class ProjectOrchestrator {
    ...
    // ---------------- Asset ops ----------------
    async addFileAsset(projectId, sourcePath, userCaption) {
    ...
    async addInlineAsset(projectId, content, type, userCaption) {
        await this.deps.projects.ensureDir(projectId);
        const project = await this.deps.projects.load(projectId);
     ...
     */
    async addBufferAsset(projectId, bytes, ext, userCaption) {
        await this.deps.projects.ensureDir(projectId);
        const project = await this.deps.projects.load(projectId);
    ...
    // ---------------- Template / variables ----------------
    async setTemplate(projectId, templateId) {
        const project = await this.deps.projects.load(projectId);
    ...
    async setVariables(projectId, vars) {
        const project = await this.deps.projects.load(projectId);
    ...
    async setVariable(projectId, key, value) {
        const project = await this.deps.projects.load(projectId);
    ...
    async setAgent(projectId, agentId, agentModel) {
        const project = await this.deps.projects.load(projectId);
        ...
        if (agentModel !== undefined)
            project.agentModel = agentModel;
        else
            project.agentModel = null;
        await this.deps.projects.save(project);
        return project;
    }
    /**
     ...
     */
    async writePreviewHtmlRaw(projectId, html) {
        const project = await this.deps.projects.load(projectId);
     ...
     */
    async writeContentGraph(projectId, graph, opts = {}) {
        const result = validateGraph(graph);
        if (!result.ok) {
            throw new HtmlVideoError('invalid-input', `ContentGraph invalid: ${result.errors.map((e) => e.message).join('; ')}`);
        }
        const project = await this.deps.projects.load(projectId);
        const projectDir = await this.deps.projects.ensureDir(projectId);
     ...
     */
    async readContentGraph(projectId) {
        const project = await this.deps.projects.load(projectId);
        if (!project.contentGraphPath)
            return null;
        const { readFile } = await import('node:fs/promises');
        const { existsSync } = await import('node:fs');
        if (!existsSync(project.contentGraphPath))
            return null;
        return JSON.parse(await readFile(project.contentGraphPath, 'utf8'));
    }
    /**
     ...
     */
    async writeFrameHtml(projectId, graphNodeId, html) {
        const project = await this.deps.projects.load(projectId);
        const graph = await this.readContentGraph(projectId);
        if (!graph) {
            throw new HtmlVideoError('invalid-input', 'Cannot write frame: project has no content graph yet');
        }
        const order = topoSort(graph);
        const idx = order.indexOf(graphNodeId);
        if (idx === -1) {
            throw new HtmlVideoError('invalid-input', `Graph node "${graphNodeId}" not found in content graph`);
        }
    ...
    async renderPreviewHtml(projectId) {
        const project = await this.deps.projects.load(projectId);
        ...
        if (!adapter.renderToHtml) {
            throw new HtmlVideoError('render-failed', `Engine ${tmpl.engine} adapter does not support renderToHtml()`);
        }
        const projectDir = await this.deps.projects.ensureDir(projectId);
            ...
            for (let i = 0; i < ordered.length; i++) {
                ...
                await adapter.render({
                    ...
                    ...(args.onProgress !== undefined && {
                        onProgress: (pct, stage) => args.onProgress((i + pct / 100) / ordered.length * 100, `frame ${i + 1}/${ordered.length}: ${stage}`),
                    }),
     ...
     */
    resolveFrameTemplateRef(f, projectEngine) {
        if (f.engine === 'remotion' && f.nativeTemplateId) {
            const nt = this.deps.templates.get(f.nativeTemplateId);
            if (!nt.native?.compositionId) {
                throw new HtmlVideoError('template-invalid', `Native template "${f.nativeTemplateId}" has no native.compositionId in its metadata`);
            }
            if (!nt.__dir) {
                throw new HtmlVideoError('template-invalid', `Native template "${f.nativeTemplateId}" has no __dir; was it loaded via TemplateRegistry?`);
            }
            return {
                engine: 'remotion',
                templateRef: {
     ...
     */
    async enhanceFrameNative(projectId, graphNodeId, nativeTemplateId) {
        const project = await this.deps.projects.load(projectId);
        const graph = await this.readContentGraph(projectId);
        if (!graph) {
        ...
        if (node.kind !== 'data') {
            throw new HtmlVideoError('invalid-input', `Frame "${graphNodeId}" is a ${node.kind} node; native data enhancement only applies to data frames`);
        }
        const tmpl = this.deps.templates.get(nativeTemplateId); // throws if unknown
        if (tmpl.engine !== 'remotion' || !tmpl.native?.compositionId) {
            throw new HtmlVideoError('invalid-input', `Template "${nativeTemplateId}" is not a native Remotion template`);
        }
        const data = normalizeRollupData(node.data);
        const frame = (project.frames ?? []).find((f) => f.graphNodeId === graphNodeId);
        if (!frame) {
            throw new HtmlVideoError('invalid-input', `Frame "${graphNodeId}" has not been rendered yet (no FrameRecord)`);
        }
     ...
     */
    async unenhanceFrame(projectId, graphNodeId) {
        const project = await this.deps.projects.load(projectId);
        const frame = (project.frames ?? []).find((f) => f.graphNodeId === graphNodeId);
        if (!frame) {
     ...
     */
    async applySoundtrack(project, outputPath, videoDurationSec, onProgress) {
        const st = project.soundtrack;
        if (!st || (!st.musicAssetId && !st.narrationAssetId))
            return;
        const findPath = (id) => id ? project.assets.find((a) => a.id === id)?.path : undefined;
        const musicPath = findPath(st.musicAssetId);
        const narrationPath = findPath(st.narrationAssetId);
        if (!musicPath && !narrationPath)
            return; // referenced assets are gone
        ...
        const defaultFadeOut = musicPath && videoDurationSec && videoDurationSec > 2
 ...
 */
function normalizeRollupData(raw) {
    const asItems = (arr) => {
        if (!Array.isArray(arr)) {
            throw new HtmlVideoError('invalid-input', 'Data frame has no `items` array to animate; expected {items:[{label,value}]}');
        }
        const items = arr
            .map((it) => {
            const o = (it ?? {});
        ...
        if (items.length === 0) {
            throw new HtmlVideoError('invalid-input', 'Data frame items had no usable {label, numeric value} pairs');
        }
        return items;
    };
    if (Array.isArray(raw))
        return { items: asItems(raw) };
    const o = (raw ?? {});
    const out = {
...
async function concatFramesWithFfmpeg(frameMp4s, outputPath, workDir, opts = {}) {
    ...
    await new Promise((resolveFn, reject) => {
        ...
        proc.on('error', (err) => {
            if (err.code === 'ENOENT') {
                reject(new HtmlVideoError('render-failed', 'ffmpeg not found on PATH. Install with `brew install ffmpeg` (macOS) or your platform equivalent.'));
            }
            else {
                reject(err);
            }
        ...
        proc.on('exit', (code) => {
            if (code === 0)
                ...
                reject(new HtmlVideoError('render-failed', `ffmpeg concat exited with code ${code}: ${stderr.slice(-2000)}`));
        });
    });
}
/**
    ...
    await new Promise((resolveFn, reject) => {
        ...
        proc.on('exit', (code) => {
            if (code === 0)
                ...
                reject(new HtmlVideoError('render-failed', `ffmpeg audio mux exited with code ${code}: ${stderr.slice(-2000)}`));
        });
    });
}
// ---------------------------------------------------------------------------
...
function recordExport(project, outputPath) {
...
function templateRefFromMeta(meta) {
    if (!meta.__dir) {
        throw new HtmlVideoError('template-invalid', `Template ${meta.id} has no __dir set; was it loaded via TemplateRegistry?`);
    }
    return {
        id: meta.id,
        engine: meta.engine,
...
function downgradeStatus(current, target) {
