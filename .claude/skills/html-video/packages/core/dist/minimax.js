 ...
 */
export function resolveMinimaxCredentials(env = process.env) {
 ...
 */
async function postAndDecode(endpoint, body, creds, label, signal) {
    ...
    try {
        ...
        throw new HtmlVideoError('render-failed', isTimeout
            ...
            : `minimax ${label} request failed: ${msg} (check the API region — international is api.minimax.io, China is api.minimaxi.com; a key only works against its own region)`, true);
    }
    const respText = await resp.text();
    if (!resp.ok) {
        throw new HtmlVideoError('render-failed', `minimax ${label} ${resp.status}: ${truncate(respText, 240)}`, resp.status >= 500);
    }
    let data;
    try {
    ...
    if (data.base_resp && data.base_resp.status_code !== 0) {
        ...
        throw new HtmlVideoError('render-failed', `minimax ${label} api error ${code}: ${data.base_resp.status_msg || 'unknown'}${hint}`);
    }
