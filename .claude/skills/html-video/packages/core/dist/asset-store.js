...
export class AssetStore {
    ...
    static async computeId(filePath) {
        const buf = await readFile(filePath);
        return createHash('sha1').update(buf).digest('hex');
    }
    static computeInlineId(content) {
        return createHash('sha1').update(content).digest('hex');
    }
    static guessMime(filePath) {
    ...
    async addFileAsset(projectId, sourcePath, userTags = [], userCaption) {
    ...
    async addInlineAsset(projectId, content, type, userTags = [], userCaption) {
     ...
     */
    async addBufferAsset(projectId, bytes, ext, userTags = [], userCaption) {
        if (bytes.length === 0) {
