...
export async function render(input, ctx) {
    ...
    if (ctx.signal?.aborted)
        throw new HtmlVideoError('cancelled', 'Aborted');
    ...
    if (!existsSync(input.template.sourcePath)) {
        throw new HtmlVideoError('template-invalid', `Source HTML not found: ${input.template.sourcePath}`);
    }
    let totalDuration = input.config.duration === 'auto' ? 5 : Math.max(0.5, Number(input.config.duration));
    ...
    const playwright = await import('playwright').catch((err) => {
        throw new HtmlVideoError('render-failed', `playwright not installed (run \`pnpm install\` from the monorepo root). ${err instanceof Error ? err.message : err}`);
    });
        ...
        try {
            const animMs = await page.evaluate(() => {
                ...
                Array.from(document.querySelectorAll('*')).forEach((el) => {
                    ...
                    durs.forEach((d, i) => {
                        if ((iters[i] || '').trim() === 'infinite')
                            return; // ignore looping bg anims
...
function runFfmpeg(args) {
    return new Promise((resolve, reject) => {
        ...
        proc.on('error', (err) => {
            if (err.code === 'ENOENT') {
                reject(new HtmlVideoError('render-failed', 'ffmpeg not found on PATH. Install with `brew install ffmpeg` (macOS).'));
            }
            else
                reject(err);
        });
        proc.on('exit', (code) => {
            if (code === 0)
                ...
                reject(new HtmlVideoError('render-failed', `ffmpeg exited ${code}: ${stderr.slice(-2000)}`));
        });
    });
}
/**
 ...
 */
export async function renderToHtml(input, ctx) {
    if (!existsSync(input.template.sourcePath)) {
        throw new HtmlVideoError('template-invalid', `Source not found: ${input.template.sourcePath}`);
    }
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${width} ${height}" width="${width}" height="${height}">
  ...
  <text x="50%" y="${height - 80}" fill="#888" font-family="monospace" font-size="32"
    ...
    for (const v of Object.values(input.variables)) {
        if (typeof v !== 'string')
            continue;
        if (!v.includes('/.html-video/bundles/'))
            continue;
        if (!existsSync(v))
            continue;
        ...
        if (!existsSync(dest))
            await copyFile(v, dest);
        const m = /assets\/([0-9a-f]{40})\./.exec(v);
        if (m && m[1]) {
            referencedAssets.push({ assetId: m[1], usagePath: dest });
        }
    }
    const totalDuration = input.config.duration === 'auto' ? 5 : input.config.duration;
    return {
        htmlPath,
        referencedAssets,
        posterPath,
