...
async function main() {
    ...
    const PNG_1x1 = Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR4nGNgYAAAAAMAASsJTYQAAAAASUVORK5CYII=', 'base64');
    ...
    if (!existsSync(htmlPath))
        throw new Error('Preview HTML missing: ' + htmlPath);
    const content = await readFile(htmlPath, 'utf8');
    if (!content.includes('<html'))
        throw new Error('Preview HTML malformed');
    ...
    if (!existsSync(outputPath))
        throw new Error('MP4 output missing');
    ...
    const graph = {
        ...
        nodes: [
            { id: 'intro', kind: 'text', text: 'Hello world', durationSec: 2 },
            { id: 'middle', kind: 'data', data: { v: 42 }, durationSec: 4 },
            { id: 'outro', kind: 'entity', props: { logo: 'OD' }, durationSec: 3 },
        ],
        edges: [
            { from: 'intro', to: 'middle', kind: 'sequence' },
            { from: 'middle', to: 'outro', kind: 'dependency' },
        ],
    };
