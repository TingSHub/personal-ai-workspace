...
export async function projectCreate(ctx, opts) {
    if (!opts.name)
        fail('invalid-input', '--name required');
...
export async function projectShow(ctx, id) {
    const project = await ctx.orchestrator.load(id);
    ok({ project });
}
export async function projectDelete(ctx, id) {
    await ctx.orchestrator.remove(id);
    ok({ project_id: id, deleted: true });
}
export async function projectAddAsset(ctx, id, opts) {
    let project;
    if (opts.file) {
        const f = resolve(opts.file);
        if (!existsSync(f))
            fail('asset-not-found', `File not found: ${f}`);
        ...
        if (!existsSync(f))
            fail('asset-not-found', `Data file not found: ${f}`);
...
export async function projectSetVar(ctx, id, key, valueJson) {
    let value;
    try {
...
export async function projectSetVars(ctx, id, varsFile) {
    const f = resolve(varsFile);
    if (!existsSync(f))
        fail('invalid-input', `vars file not found: ${f}`);
    const vars = JSON.parse(await readFile(f, 'utf8'));
    const project = await ctx.orchestrator.setVariables(id, vars);
    ok({ project_id: project.id, variables: project.variables });
}
export async function projectPreview(ctx, id) {
...
export async function projectRender(ctx, id, opts) {
