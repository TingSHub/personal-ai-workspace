...
import { projectCreate, projectList, projectShow, projectDelete, projectAddAsset, projectRemoveAsset, projectSetTemplate, projectSetVar, projectSetVars, projectPreview, projectRender, } from './commands/project.js';
...
const cacFn = typeof cacModule === 'function' ? cacModule : cacModule.default;
...
cli.command('doctor', 'Diagnose environment').action(async (opts) => {
    setJsonMode(!!opts.json);
    const ctx = await bootstrap({ cwd: opts.cwd });
    await runDoctor(ctx);
});
cli.command('list-engines', 'List installed engine adapters').action(async (opts) => {
    setJsonMode(!!opts.json);
    const ctx = await bootstrap({ cwd: opts.cwd });
...
cli.command('project-list', 'List all projects').action(async (opts) => {
    setJsonMode(!!opts.json);
    const ctx = await bootstrap({ cwd: opts.cwd });
...
cli
    ...
    setJsonMode(!!opts.json);
    if (!opts.asset)
        fail('invalid-input', '--asset required');
    const ctx = await bootstrap({ cwd: opts.cwd });
...
cli
    ...
    setJsonMode(!!opts.json);
    if (!opts.template)
        fail('invalid-input', '--template required');
    const ctx = await bootstrap({ cwd: opts.cwd });
...
cli
    ...
    setJsonMode(!!opts.json);
    if (!opts.key)
        fail('invalid-input', '--key required');
    const ctx = await bootstrap({ cwd: opts.cwd });
...
cli
    ...
    setJsonMode(!!opts.json);
    if (!opts.varsFile)
        fail('invalid-input', '--vars-file required');
    const ctx = await bootstrap({ cwd: opts.cwd });
