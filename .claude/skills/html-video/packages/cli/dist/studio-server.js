...
export async function startStudioServer(ctx, port) {
        ...
        try {
            ...
            if (addAssetMatch && addAssetMatch[1] && m === 'POST') {
                ...
                if (ct.startsWith('multipart/form-data')) {
                    ...
                    const body = await readBody(req);
                    if (body.kind === 'text') {
                        project = await ctx.orchestrator.addInlineAsset(id, body.content ?? '', 'text', body.caption);
                    }
                    else if (body.kind === 'data') {
                        project = await ctx.orchestrator.addInlineAsset(id, body.content ?? '', 'data', body.caption);
                    }
                    else if (body.kind === 'file' && body.path) {
                        project = await ctx.orchestrator.addFileAsset(id, body.path);
                    }
                    else {
            ...
            if (tplMatch && tplMatch[1] && m === 'PUT') {
                const body = await readBody(req);
                const project = await ctx.orchestrator.setTemplate(tplMatch[1], body.template_id);
                ...
                const tmpl = ctx.templates.get(body.template_id);
            ...
            if (agentMatch && agentMatch[1] && m === 'PUT') {
                const body = await readBody(req);
                const project = await ctx.orchestrator.setAgent(agentMatch[1], body.agent_id || null, body.agent_model === undefined ? undefined : (body.agent_model || null));
                return json(res, 200, { project });
            }
            ...
            if (varsMatch && varsMatch[1] && m === 'PUT') {
                const body = await readBody(req);
                const project = await ctx.orchestrator.setVariables(varsMatch[1], body.variables ?? {});
                return json(res, 200, { project });
            }
            ...
            if (enhMatch && enhMatch[1] && enhMatch[2] && m === 'POST') {
                ...
                const nativeTemplateId = body.nativeTemplateId || 'frame-data-rollup';
            ...
            if (expMatch && expMatch[1] && m === 'POST') {
                ...
                try {
                    ...
                    const ms = Date.now() - t0;
                    process.stderr.write(`[studio:export] proj=${projectId} done in ${ms}ms → ${outputPath}\n`);
            ...
            if (genAudioMatch && genAudioMatch[1] && m === 'POST') {
                ...
                try {
                    ...
                    if (wantMusic) {
                        ...
                        const { asset } = await ctx.orchestrator.addBufferAsset(projectId, music.bytes, music.ext, `background music · ${body.music.prompt.trim().slice(0, 60)}`);
                        soundtrack.musicAssetId = asset.id;
                        soundtrack.musicPrompt = body.music.prompt.trim();
                        if (body.music.volumeDb !== undefined)
                            soundtrack.musicVolumeDb = body.music.volumeDb;
                    ...
                    if (wantNarration) {
                        ...
                        const { asset } = await ctx.orchestrator.addBufferAsset(projectId, nar.bytes, nar.ext, `narration · ${body.narration.text.trim().slice(0, 60)}`);
            ...
            if (draftNarrMatch && draftNarrMatch[1] && m === 'POST') {
                ...
                try {
                    ...
                    if (body.frameId) {
                        ...
                        if (!target)
                            return json(res, 400, { error: `frame "${body.frameId}" not in content-graph` });
                        const prompt = [
                        ...
                        allFrames.forEach((f, i) => { if (lines[i])
                            narrationByFrame[f.id] = lines[i]; });
                    }
            ...
            if (modelsMatch && modelsMatch[1] && m === 'GET') {
                ...
                if (!bin)
                    return json(res, 400, { error: 'vela binary not found' });
                try {
            ...
            if (loginMatch && loginMatch[1] && m === 'POST') {
                ...
                if (agentId !== 'amr')
                    return json(res, 400, { error: `agent "${agentId}" has no login flow` });
                const def = findAgent(agentId);
                if (!def)
                    return json(res, 404, { error: `agent "${agentId}" not registered` });
                ...
                try {
                    ...
                    const code = await new Promise((resolveCode, rejectCode) => {
                        ...
                        catch { /* */ } rejectCode(new Error('login timed out (5 min)')); }, 5 * 60_000);
                        child.on('error', (e) => { clearTimeout(timer); rejectCode(e); });
                        child.on('exit', (c) => { clearTimeout(timer); resolveCode(c ?? -1); });
                    });
                    if (code !== 0)
                        return json(res, 400, { ok: false, error: `vela login exited with code ${code}` });
            ...
            if (msgsMatch && msgsMatch[1] && m === 'POST') {
                ...
                if (ct.startsWith('multipart/form-data')) {
                    ...
                    focusFrameId = body.focus_frame_id ?? '';
                }
                ...
                for (const sourceUrl of extractUrls(userText)) {
                    try {
                        ...
                        const updated = await ctx.orchestrator.addInlineAsset(id, src.markdown, 'text', `${label}: ${src.title || sourceUrl}`);
                        ...
                        if (asset?.path) {
                            ...
                            process.stderr.write(`[studio:fetch-source] ${src.kind} ${sourceUrl} → ${src.markdown.length} chars${src.truncated ? ' (truncated)' : ''}\n`);
                        }
                    }
                    catch (e) {
                ...
                const phaseInfo = detectPhase(history, userText, !!project.templateId, attachments.some((a) => !!a.inlineText), focusFrameId);
                const t0 = Date.now();
                ...
                process.stderr.write(`[studio:msg] proj=${id} phase=${phaseInfo.phase} prompt=${fullPrompt.length}B user=${JSON.stringify(userText.slice(0, 80))} attachments=${attachments.length}\n`);
                ...
                try {
                    ...
                    const isMultiGenerate = phaseInfo.phase === 'generate' &&
                    ...
                    const isMultiFrameProject = (project.frames ?? []).length > 1 ||
                        Number(phaseInfo.inputs.collected?.frame_count ?? '1') > 1;
                    let rewriteInputs;
                    ...
                    if (isMultiGenerate || rewriteInputs) {
                        ...
                        process.stderr.write(`[studio:msg] proj=${id} phase=split-generate done text=${assistantText.length}B\n`);
                    }
                    else {
                        ...
                        process.stderr.write(`[studio:msg] proj=${id} phase=${phaseInfo.phase} done in ${elapsedMs}ms exit=${exitInfo.exitCode} text=${assistantText.length}B chunks=${textChunks}\n`);
                        ...
                        if (assistantText.trim().length < 32 && phaseInfo.phase === 'iterate' && priorHtml) {
                            ...
                            process.stderr.write(`[studio:msg] proj=${id} retry done text=${retryText.length}B\n`);
                        }
                    ...
                    let persistText = summaryLine
                        ? assistantText
                            ...
                            .replace(/```json[\s\S]*?```/i, (m) => /content-graph|"intent"\s*:|"nodes"\s*:/i.test(m) ? '' : m)
                            .trim() || summaryLine
                        : assistantText;
            ...
            if (fitMatch && fitMatch[1] && m === 'POST') {
                ...
                const byFrame = (await readBody(req)).narrationByFrame ?? {};
                const lenOf = (id) => (byFrame[id]?.trim().length ?? 0);
            ...
            if (previewServeMatch && previewServeMatch[1]) {
                ...
                if (existsSync(projAssets))
                    return serveFile(projAssets, res);
                ...
                if (project.templateId) {
                    try {
                        ...
                        if (tmpl?.__dir && sub.length > 1) {
                            ...
                            if (tmplResolved.startsWith(tmplRoot + '/') &&
 ...
 */
function injectCompositionPlayer(html) {
 ...
 */
export function decodeUploadFilename(star, plain) {
    if (star) {
...
async function receiveMultipart(req, contentType) {
...
async function receiveMultipartFile(req, contentType) {
...
async function loadMessages(ctx, projectId) {
...
async function saveMessages(ctx, projectId, messages) {
    const projectDir = await ctx.projects.ensureDir(projectId);
    const filePath = join(projectDir, 'messages.json');
    const fs = await import('node:fs/promises');
...
function isFromTemplateStyle(style) {
    return /^从设计模板选|design template|pick.*template|from template/i.test(style.trim());
}
function detectPhase(history, userText, hasTemplate, hasSourceMaterial = false, focusFrameId = '') {
    ...
    if (prev.kind === 'content-question') {
        ...
        const isFreeRein = trimmed.length <= 16 &&
...
function hasEnoughContent(history, pending) {
...
function lastAssistantCardWithMeta(history) {
    for (let i = history.length - 1; i >= 0; i--) {
        const m = history[i];
        if (m.role !== 'assistant')
            continue;
        const c = m.content;
        if (!c.trim() || /^⚠️/.test(c.trim()))
            continue;
...
function lastCardPickByPhase(history, phase) {
 ...
 */
function resolveOpeningTopic(project, history) {
    const fromIntent = project.intent?.trim();
    if (fromIntent)
        return fromIntent.slice(0, 200);
...
function lastFormSubmission(history) {
    for (let i = history.length - 1; i >= 0; i--) {
        const m = history[i];
        if (m.role !== 'user')
            continue;
...
function hadGenerationYet(history) {
...
function lastAssistantAskedFormat(history) {
    for (let i = history.length - 1; i >= 0; i--) {
        ...
        const c = m.content;
        if (!c.trim() || /^⚠️/.test(c.trim()))
            continue; // skip empty / warning turns
 ...
 */
export function parseFormatReply(text) {
    const t = text.trim();
    if (!t || t.length > 80)
        return undefined; // long text is content, not a format answer
    const out = {};
    ...
    if (ratioNorm === '16:9' || /横屏|landscape|宽屏/i.test(t))
        out.aspect = '16:9 横屏';
    else if (ratioNorm === '9:16' || /竖屏|手机|portrait|vertical/i.test(t))
        out.aspect = '9:16 手机竖屏';
    else if (ratioNorm === '1:1' || /方形|square/i.test(t))
        out.aspect = '1:1 方形';
...
function isDesignSpec(a) {
    const name = (a.filename || '').toLowerCase();
    if (/(^|\/)(design|frame)\.md$/.test(name) || /\bframe\.md\b|\bdesign\.md\b/.test(name))
        return true;
...
function renderDesignSpecBlock(specs) {
    if (!specs.length)
        return [];
    const out = [
...
function parseGraphJsonTolerant(raw) {
    try {
    ...
    const repaired = noTrailing.replace(/("(?:synopsis|text)"\s*:\s*")([\s\S]*?)("\s*(?:,|\}|\]))/g, (_m, pre, val, post) => pre + val.replace(/\\?"/g, '\\"') + post);
...
function buildHtmlGenerationPrompt(args) {
    ...
    if (phase === 'opener') {
        const opener = [];
        opener.push(`The user just opened a project and said "${trimmed}". You are an HTML-video creation assistant.`);
        opener.push('');
        ...
        opener.push('');
        if (tmpl) {
            opener.push(`Note: a template "${tmpl.name}" is currently selected (${tmpl.description}). Treat it as a visual style reference only — content type still drives the structure.`);
            opener.push('');
        }
    ...
    if (phase === 'confirm') {
        ...
        const summaryRows = [];
        if (pickedType)
            summaryRows.push({ label: '类型', value: pickedType });
        if (contentTurns.length > 0) {
            summaryRows.push({ label: '内容', value: contentTurns.join(' · ').slice(0, 240) });
        }
        if (pickedStyle)
            summaryRows.push({ label: '风格', value: pickedStyle });
        if (tmpl)
            summaryRows.push({ label: '模板', value: tmpl.name });
        const labelMap = {
        ...
        for (const k of keys) {
            const v = collected[k];
            if (v)
                summaryRows.push({ label: labelMap[k] ?? k, value: k === 'per_frame' ? `${v}s` : v });
        }
        if (pf > 0) {
        ...
        const topicThin = attachments.length === 0 &&
...
<div class="stage"><h1 data-hv-text="headline">PLACEHOLDER</h1></div>
    ...
    if (baseHtml) {
        ...
        if (summary.headline)
            it.push(`Headline: ${summary.headline}`);
...
function summariseHtmlForIterate(html) {
    const subheads = [];
    const dataPoints = [];
    ...
    while ((m = textRe.exec(html)) !== null) {
        ...
        if (!val)
            continue;
        if (/headline|title|hero/i.test(key) && !headline)
            headline = val;
        else if (/data|stat|value|number/i.test(key))
            dataPoints.push(`${key}: ${val}`);
        else
            subheads.push(`${key}: ${val}`);
    }
    // Body / stage background colour (rough)
    const bgColors = Array.from(html.matchAll(/background[^:]*:\s*(#[0-9a-f]{3,8}|rgb[a]?\([^)]+\)|hsla?\([^)]+\))/gi)).slice(0, 3).map((x) => x[1]).filter(Boolean);
    // Font families (first occurrence in css)
    const fontFamilies = Array.from(new Set(Array.from(html.matchAll(/font-family\s*:\s*([^;}]+)/gi))
 ...
 */
function extractContentGraphAndFrames(text) {
    ...
    let graph;
    try {
        graph = parseGraphJsonTolerant(graphMatch[1].trim());
    }
    catch {
        return null;
    }
...
async function runSplitMultiFrameGenerate(args) {
    ...
    if (restyleOnly) {
        ...
        if (styleLabel)
            graphPromptParts.push(`- 风格 / style: ${styleLabel}`);
    ...
    for (let i = 0; i < graph.nodes.length; i++) {
        ...
        if (frameSpecs.length > 0)
            fp.push(...renderDesignSpecBlock(frameSpecs));
...
function describeNode(node) {
    const bits = [];
    if (node.label)
        bits.push(node.label);
    if (node.text)
        bits.push(`text: ${node.text.slice(0, 200)}`);
    if (node.kind === 'data' && node.data !== undefined) {
        bits.push(`data: ${JSON.stringify(node.data).slice(0, 200)}`);
    }
    if (node.kind === 'entity' && node.props !== undefined) {
        bits.push(`entity props: ${JSON.stringify(node.props).slice(0, 200)}`);
    }
    if (node.frameIntent)
        bits.push(`intent: ${node.frameIntent}`);
    if (bits.length === 0)
        bits.push(`(${node.kind} frame "${node.id}")`);
    return bits.join('; ');
}
/** Spawn the agent, collect all stdout text, return when done. */
async function callAgentSimple(def, prompt, cwd, model) {
    let buf = '';
    const handle = spawnAgent({
        def,
        prompt,
