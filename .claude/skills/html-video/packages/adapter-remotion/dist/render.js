...
function loadRemotion() {
    try {
        ...
        throw new EngineError('engine-not-installed', `Remotion is not installed. Run \`pnpm add remotion @remotion/bundler @remotion/renderer react react-dom\` in the workspace root. (${err instanceof Error ? err.message : err})`);
    }
}
...
const bundleCache = new Map(); // entryPath -> serveUrl
async function bundleOnce(bundle, entry, ctx) {
 ...
 */
export function neutralizeBlockingResources(html) {
    return html.replace(/<link\b[^>]*\brel=(["']?)stylesheet\1[^>]*>/gi, (tag) => {
        ...
        if (!/\bhref=(["'])(?:https?:)?\/\//i.test(tag))
            return tag;
        if (/\bmedia=/i.test(tag))
            return tag; // already has media handling — leave it
...
export async function render(input, ctx) {
    ...
    if (!sourcePath || !existsSync(sourcePath)) {
        throw new EngineError('template-invalid', `${isNative ? 'Native template entry' : 'Source HTML'} not found: ${sourcePath}`);
    }
    ...
    if (isNative) {
        ...
        const inputProps = { data, width, height, ...restVars };
...
function remotionVersion() {
    try {
        return require('remotion/package.json').version;
    }
    catch {
        return '4.x';
    }
