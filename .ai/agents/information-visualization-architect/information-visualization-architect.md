...
| 辅助脚本 | scripts/check_ev_refs.py（visual-brief EV 引用 ↔ evidence-reference 可解析性检查，输入前/输出后运行）；projects/investment-research-video/scripts/check_podcast_visual_sync.py（多步图表 × 真实 turn 的 narration_beats 同步门禁，执行层 Phase 2 使用） |
...
- 踩坑（2026-09-07 猪周期）：多步图只在 topic 起始一次性绘制、不与口播同步——已在实体定义 §3 写入 narration_beats 契约；渲染器实现逐点 reveal 前，多步图一律不通过同步门禁。
