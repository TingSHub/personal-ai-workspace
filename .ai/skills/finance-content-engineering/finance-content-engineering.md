...
| 经验引用 | huashu-test；notes-spoken-polish；published-video-dialogue-caption-visual-sync |
...
- **来源口播最小化（2026-09-07 猪周期踩坑）**：事实默认直接陈述，来源由 evidence ID、字幕/画面脚注和发布来源清单承担；只有预测、观点、估计、争议口径、来源身份影响结论或对比不同口径时，才在口播中点名主体。“公开报告认为”不能当事实句默认前缀。
- **数字双文本**：口播改写出 `text`（读音安全读法，如“百分之六点五”）的同时，必须同时产出 `display_text`（观众展示，如“6.5%”，优先阿拉伯数字与单位），两者数值等价；禁止为读音安全把展示文本写成中文读法。
- **问句语气颗粒（2026-09-07 猪周期）**：问号不是 TTS 升调依据。疑问句要带句尾语气词（吗/呢/吧/么/呀/啊）或疑问语汇结尾（什么/为什么/怎么看/怎么读/怎么办/在哪里/哪儿/哪个/对不对/是不是/对吗/多少）；“是不是反转了？”这类裸疑问结构要改写成“……是不是已经反转了呢？”。
