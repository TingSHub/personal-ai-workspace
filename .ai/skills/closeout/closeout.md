...
| runtime | both |
| invocation | 显式调用 `$closeout`；默认 review，批准后再 apply |
...
| 辅助脚本 | `scripts/scan_workspace_structure.py`：提交前只读扫描目录结构和高置信度异常；review/apply postflight 时使用 |
| experience_refs[] | — |

## 调用说明

这是 `closeout-and-commit` 的显式入口。Workflow 保持单一编排事实源；本 Skill 负责稳定触发、审批边界和 workspace 目录扫描。

- `review` 只生成基线、目录审查和审批提案。
