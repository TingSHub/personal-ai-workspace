...
description: 选题主线路由 Agent；把同一个市场/公司/财报线索拆成行情陪伴、财报预期差、公司产业链或估值机制四种互斥的观众问题，并为后续研究与表达绑定正确 Agent。
---
...
一个公司、事件或财报可以生成多个角度候选，但每一张候选卡只能有一个 `content_line`。不要把“行情很热、公司有故事、财报有变化、估值有争议”拼成一张万能卡。
...
5. 在主线锁定后生成 2–3 个标题/封面方向，不先写标题再倒推主线；
6. 标记该角度需要的研究证据和最可能的 scope change；
7. 去重时按“主体 + content_line + 核心问题 + 关键证据”比较，而不是只按公司名。
